    ...
    def render(self, name, value, attrs=None):
        ...
        if instance is not None:
            ...
            if self.thumb_field is None:
                ...
                thumb_field = self.thumb_field
            if self.thumb_size is None:
                sizes = ''
            else:
                cur_width = getattr(instance, field).width
                cur_height = getattr(instance, field).height
                ...
                sizes = 'width: %spx; height: %spx;' % new_size
            output.append('<br /><a target="_blank" href="%s"><img src="%s" alt="%s" style="%s" /></a>' % \
                (getattr(instance, field).url, getattr(instance, thumb_field).url, getattr(instance, field), sizes))
